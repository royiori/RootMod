...
public:
   TSubString(const TSubString &_s)
     : fStr(_s.fStr), fBegin(_s.fBegin), fExtent(_s.fExtent) { }
...
protected:
   ...
   enum { kAlignment = 16 };
   static Ssiz_t  Align(Ssiz_t _s) { return (_s + (kAlignment-1)) & ~(kAlignment-1); }
   static Ssiz_t  Recommend(Ssiz_t _s) { return (_s < kMinCap ? kMinCap : Align(_s+1)) - 1; }
   static Ssiz_t  AdjustCapacity(Ssiz_t oldCap, Ssiz_t newCap);

private:
   Bool_t         IsLong() const { return Bool_t(fRep.fShort.fSize & kShortMask); }
#ifdef R__BYTESWAP
   void           SetShortSize(Ssiz_t _s) { fRep.fShort.fSize = (unsigned char)(_s << 1); }
...
#endif
   void           SetLongSize(Ssiz_t _s) { fRep.fLong.fSize = _s; }
   Ssiz_t         GetLongSize() const { return fRep.fLong.fSize; }
   void           SetSize(Ssiz_t _s) { IsLong() ? SetLongSize(_s) : SetShortSize(_s); }
   void           SetLongCap(Ssiz_t _s) { fRep.fLong.fCap = kLongMask | _s; }
...
inline UInt_t Hash(const TString &_s) { return _s.Hash(); }
inline UInt_t Hash(const TString *_s) { return _s->Hash(); }
...
inline TString &TString::Append(const TString &_s)
{ return Replace(Length(), 0, _s.Data(), _s.Length()); }

inline TString &TString::Append(const TString &_s, Ssiz_t n)
{ return Replace(Length(), 0, _s.Data(), TMath::Min(n, _s.Length())); }

inline TString &TString::operator+=(const char *cs)
{ return Append(cs, cs ? strlen(cs) : 0); }

inline TString &TString::operator+=(const TString &_s)
{ return Append(_s.Data(), _s.Length()); }
...
inline Bool_t TString::BeginsWith(const char *_s, ECaseCompare cmp) const
{ return Index(_s, _s ? strlen(_s) : (Ssiz_t)0, (Ssiz_t)0, cmp) == 0; }
...
inline Bool_t TString::Contains(const char *_s, ECaseCompare cmp) const
{ return Index(_s, _s ? strlen(_s) : 0, (Ssiz_t)0, cmp) != kNPOS; }
...
inline Ssiz_t TString::Index(const char *_s, Ssiz_t i, ECaseCompare cmp) const
{ return Index(_s, _s ? strlen(_s) : 0, i, cmp); }

inline Ssiz_t TString::Index(const TString &_s, Ssiz_t i, ECaseCompare cmp) const
{ return Index(_s.Data(), _s.Length(), i, cmp); }
...
inline TString &TString::Insert(Ssiz_t pos, const TString &_s)
{ return Replace(pos, 0, _s.Data(), _s.Length()); }

inline TString &TString::Insert(Ssiz_t pos, const TString &_s, Ssiz_t n)
{ return Replace(pos, 0, _s.Data(), TMath::Min(n, _s.Length())); }
...
inline TString &TString::Prepend(const TString &_s)
{ return Replace(0, 0, _s.Data(), _s.Length()); }

inline TString &TString::Prepend(const TString &_s, Ssiz_t n)
{ return Replace(0, 0, _s.Data(), TMath::Min(n, _s.Length())); }
...
inline TString &TString::Replace(Ssiz_t pos, Ssiz_t n, const TString& _s)
{ return Replace(pos, n, _s.Data(), _s.Length()); }

inline TString &TString::Replace(Ssiz_t pos, Ssiz_t n1, const TString &_s,
                                 Ssiz_t n2)
{ return Replace(pos, n1, _s.Data(), TMath::Min(_s.Length(), n2)); }
...
inline TSubString &TSubString::operator=(const TSubString &_s)
{ fStr = _s.fStr; fBegin = _s.fBegin; fExtent = _s.fExtent; return *this; }
